...
func testWebServers(t *testing.T, context spec.G, it spec.S) {
	...
	context("detects a HTTPD app", func() {
		...
		it("builds successfully", func() {
			var err error
			source, err = occam.Source(filepath.Join("testdata", "web-servers", "httpd-sample"))
			Expect(err).NotTo(HaveOccurred())
	...
	context("detects a JavaScript frontend app", func() {
		...
		context("app uses react and httpd", func() {
			it("builds successfully", func() {
				var err error
				source, err = occam.Source(filepath.Join("testdata", "web-servers", "react-frontend-sample"))
				Expect(err).NotTo(HaveOccurred())
